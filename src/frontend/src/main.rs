...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            "/cart",
            get(handlers::cart::get_cart).post(handlers::cart::post_cart),
        )
        .route("/cart/empty", post(handlers::cart::post_cart_empty))
        .route("/cart/checkout", post(handlers::cart::post_cart_checkout))
